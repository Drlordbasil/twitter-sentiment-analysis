...
def get_tweets(api, query, count=100):
    ...
    try:
        ...
        tweets = [{'text': tweet.text, 'sentiment': get_tweet_sentiment(tweet.text)} for tweet in fetched_tweets]
        return tweets
...
def preprocess_tweets(tweets):
    ...
    cleaned_tweets = []
    for tweet in tweets:
        cleaned_tweet = ''.join(char for char in tweet['text'] if char.isalnum() or char.isspace())
...
if __name__ == "__main__":
    query = input("Enter a keyword to perform sentiment analysis on social media posts: ")
